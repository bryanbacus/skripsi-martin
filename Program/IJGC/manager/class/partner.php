...
class partner extends koneksi{
	...
	function Kategori($kategori){
		$sql = "select id,description from m_partner order by id asc";
		$row = $this->exQ($sql);
		...
		if(isset($_POST['simpan'])){
			if($_POST['simpan']){
				if(!detectBlank($_POST)){
					...
					$name = eraseStrange($_POST['name']);
					$content = eraseStrange($_POST['content']);
					$status = preg_replace("@[^0-9]@i","",$_POST['status']);
					if($status=="")	$status = "0";
					$sql = "insert into tbl_partner(name,description,link,type_partner,status)
							values('$name','$content','$link','$kategori','$status')";
					#echo $sql;
					if($this->exQ($sql)){
						$this->pesan = "Partner telah ditambahkan !";
						return true;
					}else{
						#echo mysql_error();
		...
		if(isset($_POST['simpan'])){
			if($_POST['simpan']){
				if(!detectBlank($_POST)){
					...
					$status = preg_replace("@[^0-9]@i","",$_POST['status']);
					if($status=="")	$status = "0";
					$sql = "update tbl_partner set name='$name', description='$content',link='$link',type_partner='$kategori',
					status='$status' where id=$id";
					#echo $sql;
					if($this->exQ($sql)){
						$this->pesan = "Partner telah diedit !";
						return true;
					}else{
						echo mysql_error();
						#redirect("../error.php?p=1");
	...
	function uploadFile($max,$nfile){
		if($_FILES['gambar']['size']>$max){
			...
			$uploaddir = FTP_PATH.'/images/partner';
			...
			if ($ftpRes){
				...
				$this->pesan = "<br><u><b>Catatan</b></u> : Gambar gagal diupload. Anda dapat mencoba meng-upload-nya kembali dengan memilih <i><b>edit</b></i> pada partner yang bersangkutan.";
				return false;
			}
		}
	}
