...
class course_factory{
	...
	function remove_course_tee($course_subid){
		$sub = trim($course_subid);
		if($sub != ""){
			$SQL = "delete from m_course_length where course_sub_id = $sub";
			$this->SimpleDB->connect();
			$this->SimpleDB->execute($SQL);
			$this->SimpleDB->disconnect();
		}	
	...
	function uploadFile($max, $course_id){
		if($_FILES['gambar']['size'] > $max){
			return false;
		}else{
		
			$file = $course_id . basename($_FILES['gambar']['name']);
			$uploaddir = FTP_PATH.'/images/course/';
		
			$result_array = getimagesize($_FILES['gambar']['tmp_name']);
			if ($result_array === false) return false;
			...
			if ($ftpRes){
				if(trim($course_id) != ""){
					$SQL = "update m_course set course_logopath='../images/course/$file' where course_id = $course_id";
					$this->SimpleDB->connect();
					$this->SimpleDB->execute($SQL);
					$this->SimpleDB->disconnect();
				}
