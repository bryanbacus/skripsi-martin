...
class foundation extends koneksi{
	
	var $pesan = "";
	var $thumbName = "";

	function Kategori($kategori){
		$sql = "select id,description from m_foundation order by id asc";
		$row = $this->exQ($sql);
	...
	function ListKategori(){
		...
		$x=0;
		while($data = mysql_fetch_array($row,MYSQL_BOTH)){
			$ListKategori[$x]['id'] = $data['id'];
			$ListKategori[$x]['description'] = $data['description'];
			$x++;
		}
		return $ListKategori;		
	}
		...
		if(isset($_POST['simpan'])){
			if($_POST['simpan']){
				if(!detectBlank($_POST)){
					...
					if($status=="")	$status = "0";
					$sql = "insert into tbl_foundation(deskripsi, link, kategori, status)
							values('$content','$link','$kategori','$status')";
					#echo $sql;
					if($this->exQ($sql)){
						$this->pesan = "Foundation telah ditambahkan !";
						return true;
					}else{
						#echo mysql_error();
		...
		if(isset($_POST['simpan'])){
			if($_POST['simpan']){
				if(!detectBlank($_POST)){
					...
					$status = preg_replace("@[^0-9]@i","",$_POST['status']);
					if($status=="")	$status = "0";
					$sql = "update tbl_foundation set kategori='$kategori', deskripsi='$content', link='$link',
							status='$status' where id=$id";
					#echo $sql;
					if($this->exQ($sql)){
						$this->pesan = "Fondation telah diedit !";
						return true;
					}else{
						echo mysql_error();
						#redirect("../error.php?p=1");
