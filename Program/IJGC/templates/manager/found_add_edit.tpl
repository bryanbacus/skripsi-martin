...
{/if}
{if $kembali}
<a href="{$this_page}?aksi=found&aksi2=edit&id={$id}">Edit Foundation kembali</a>
{/if}
{if !$dShowMe}
	...
	<form method="post" name="addLinks" onSubmit="return submitForm();" enctype="multipart/form-data">
		<table border="0">
			...
			<tr>
				...
				<td class="texton">:</td>
				<td class="texton"><input type="text" name="link" value="{$link}" size="50" maxlength="255" /></td>
			</tr>
			<tr>
			...
			<tr>
				...
				{section name=list loop=$ListKategori}
					{if $ListKategori[list].id eq $kategori}
						<option value="{$ListKategori[list].id}" selected="selected">{$ListKategori[list].description}</option>
					{else}
						<option value="{$ListKategori[list].id}">{$ListKategori[list].description}</option>
					{/if}
				{/section}
				</select>
				</td>
