...
{/if}
{if $kembali}
<a href="{$this_page}?aksi=news&aksi2=edit&id={$id}">Edit Berita kembali</a>
{/if}
{if !$dShowMe}
	...
	<form method="post" name="addLinks" onSubmit="return submitForm();" enctype="multipart/form-data">
		...
		<table border="0">
			<tr>
				...
				<td class="texton">:</td>
				<td class="texton"><input type="text" name="jdl" value="{$jdl}" size="50" maxlength="255" /></td>
			</tr>
			<tr>
				<td class="label">TANGGAL</td>
				<td class="texton">:</td>
				<td class="texton"><script>DateInput('tgl', true, 'MM-DD-YYYY','{$tgl}')</script></td>
			</tr>
			<tr>
				<td class="label" valign="top">CUPLIKAN</td>
				<td class="texton" valign="top">:</td>
				<td class="texton">
					<textarea name="cuplikan" class="text_area" cols="30" rows="4">{$cuplikan}</textarea>
					<br>
					max. 300 karakter
				</td>
			</tr>
			<tr>
				<td class="label" valign="top">ISI</td>
				<td class="texton" valign="top">:</td>
				<td class="texton">
					<script language="javascript">
						...
						function submitForm() {
							...
							updateRTE('berita'); //use this when syncing only 1 rich text editor ("rtel" is name of editor)
					...
					<!--
						...
						writeRichText('berita', isi, 400, 200, true, false);
						//-->
						//-->
					</script>
				</td>
