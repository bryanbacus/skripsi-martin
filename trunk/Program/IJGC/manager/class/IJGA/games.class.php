...
class games_factory{
	...
	function create_games($games){
		...
		if($games->members_id != ""){
			...
			$games->games_id = "G".$games->games_type.date("YmdHis").$games->id_round_tour."00".$games->id_player.mt_rand();
		}
		...
		$SQL .= "  '".$games->members_group."',";
		$SQL .= "  ".$games->members_age.",";
		$SQL .= "  ".$games->id_round_tour.",";
		$SQL .= "  ".$games->id_player.")";
	...
	function getGamesList($date_first = "", $date_last = "", $limit = "", $offset = "", $column = "", $value = ""){
		$this->SimpleDB->connect();
