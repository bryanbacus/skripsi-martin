...
class tips extends koneksi{
	
	var $pesan = "";
	var $thumbName = "";

	function Kategori($kategori){
		$sql = "select id,description from m_tips order by id asc";
		$row = $this->exQ($sql);
		...
		if(isset($_POST['simpan'])){
			if($_POST['simpan']){
				if(!detectBlank($_POST)){
					...
					if($status=="")	$status = "0";
					$sql = "insert into tbl_tips(kategori, content, link, status)
							values('$kategori','$content','$link','$status')";
					#echo $sql;
					if($this->exQ($sql)){
						$this->pesan = "Tips telah ditambahkan !";
						return true;
					}else{
						#echo mysql_error();
